    ...
    <div v-else>
      ...
      <div class="pipeline">
       <a class="pipeline-id-link" target="_blank" rel="noopener noreferrer" :href="project.web_url + '/pipelines/' + pipeline.id">
        ...
        <div v-for="stage in stages" class="jobs">
          <job-view v-for="job in jobsByStages[stage]" :key="job.id" :job="job" :project="project" />
        </div>
    ...
    methods: {
      ...
      sortJobsByStages() {
				for (var i = 0; i < jobs.length; i++)	{
					if (!stages.includes(jobs[i].stage)) {
							stages.push(jobs[i].stage);
					}
				}
				for (var i = 0; i < stages.length; i++) {
					jobsByStage[stages[i]] = jobs.filter(job => job.stage === stages[i]);
				}
      },
